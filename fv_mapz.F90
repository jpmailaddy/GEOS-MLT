...
!$OMP                          private(gz,cvm,kp,k_next,bkh,dp2,blend_factor,kappa_blend,cp_blend,  &
   ...
   if ( hydrostatic ) then
      ...
      if ( GEOS_MLT ) then     
         ...
         do k=1,km
            do i=is,ie
               if (k .lt. mol_diffusion_k_top) then
                  pkz(i,j,k) = (pk(i,j,k+1)-pk(i,j,k)) / (Kappa_MLT(i,j,k)*(peln(i,k+1,j)-peln(i,k,j)))
               else if (k .le. mol_diffusion_k_top + 2) then
                  blend_factor = real(k - mol_diffusion_k_top) / 3.0
                  kappa_blend = (1.0 - blend_factor)*Kappa_MLT(i,j,k) + blend_factor*akap
                  pkz(i,j,k) = (pk(i,j,k+1)-pk(i,j,k)) / (kappa_blend*(peln(i,k+1,j)-peln(i,k,j)))
               else
...
!$OMP                               te_2d,te,delp,hydrostatic,GEOS_MLT,mol_diffusion_k_top,mol_diffusion_k_bot,hs,rg,pt,peln, adiabatic, &
